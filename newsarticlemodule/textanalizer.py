...
def get_bags_of_words(text):
    ...
    return [word for word in filter(lambda w: acceptable_word(w), correct_word)]
...
def get_entites(words):
    for word in words.ents:
        yield (normalise(word.text), word.label_)


def get_named_entity(text):
    doc = nlp(remove_white_signs(text))

    tuples = get_entites(doc)
    # print([(X[0], X[1]) for X in tuples])
    return tuples


def remove_white_signs(text):
    return remove_special_signs(text.replace("\n", " ").replace("\ t", " ").replace("\t", " ").replace("\\", ""))
